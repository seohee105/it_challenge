...
func set_character(name: String) -> void:
	_tex_idle = _load("res://assets/boss_battle/%s_idle.png" % name)
	if _tex_idle == null:
		if name != "player":
			set_character("player")
		return
	_tex_attack = _load("res://assets/boss_battle/%s_attack.png" % name)
	_tex_hit = _load("res://assets/boss_battle/%s_hit.png" % name)
	if _tex_attack == null:
