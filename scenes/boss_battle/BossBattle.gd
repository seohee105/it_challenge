...
const PlayerViewScript := preload("res://scenes/boss_battle/PlayerView.gd")
const BossViewScript := preload("res://scenes/boss_battle/BossView.gd")
const FxScript := preload("res://scenes/boss_battle/Fx.gd")
const SfxScript := preload("res://scenes/boss_battle/Sfx.gd")
const ArenaBgScript := preload("res://scenes/boss_battle/ArenaBg.gd")
const PixelsData := preload("res://scenes/boss_battle/Pixels.gd")
const PixelSpriteScene := preload("res://scenes/boss_battle/PixelSprite.gd")
...
# id는 HomeScene의 던전 선택 id(fast_food/dessert/salad)와 맞춰서, 어느 맵에서
# 왔는지에 따라 정확히 그 보스로 고정 입장시키기 위한 값이다.
const BOSSES := [
	{ "id": "fast_food", "name": "메가 버거 타이탄", "spr": "burger", "tex": "burger", "hp": 2700, "interval": 2.3, "attacks": [
	...
	{ "id": "dessert", "name": "슈가 퀸", "spr": "sugar_queen", "tex": "queen", "hp": 2450, "interval": 2.1, "attacks": [
	...
	{ "id": "salad", "name": "시저 샐러드 킹", "spr": "salad_king", "tex": "king", "hp": 2900, "interval": 2.3, "attacks": [
...
func _ready() -> void:
	...
	var sv0 := get_node_or_null("/root/BossSave")
	if sv0:
	...
	for n in boss_files:
		_boss_tex[n] = _load_tex("res://assets/boss_battle/%s.png" % boss_files[n])
		_boss_tex_enrage[n] = _load_tex("res://assets/boss_battle/%s_enrage.png" % boss_files[n])
		_boss_tex_defeat[n] = _load_tex("res://assets/boss_battle/%s_defeat.png" % boss_files[n])
	for n in ["fries", "cola", "cheese", "lollipop", "cake", "donut", "cream", "fork", "bacon", "dressing"]:
		_atk_tex[n] = _load_tex("res://assets/boss_battle/atk_%s.png" % n)
	_build_ui()
	sfx = SfxScript.new()
	add_child(sfx)
...
func start_battle() -> void:
	...
	if GameManager.pending_boss_id != "":
		for idx in BOSSES.size():
			if BOSSES[idx].id == GameManager.pending_boss_id:
				bi = idx
				break
	...
	boss_view.reset()
	character_view.set_sprite(BossSave.char_id)   # 저장된 유저 캐릭터 적용 (전환 구조)
...
func _end_battle(win: bool) -> void:
	...
	use_btn.pressed.connect(func() -> void:
		var s := get_node_or_null("/root/BossSave")
